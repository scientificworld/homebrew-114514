...
# biliup-rs
data = retry_util(lambda: requests.get("https://api.github.com/repos/biliup/biliup/releases/latest").json())
if list(filter(lambda x: x["name"].startswith("biliupR"), data["assets"])):
